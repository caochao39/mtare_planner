...
{
  std::vector<int> vec1_copy = vec1;
  std::vector<int> vec2_copy = vec2;

  std::sort(vec1_copy.begin(), vec1_copy.end());
  std::sort(vec2_copy.begin(), vec2_copy.end());

  vec_common.clear();

  std::set_intersection(vec1_copy.begin(), vec1_copy.end(), vec2_copy.begin(), vec2_copy.end(),
                        std::back_inserter(vec_common));
}
