  ...
  , robot_position_update_(false)
{
  ...
  collision_cloud_kdtree_ = pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr(new pcl::KdTreeFLANN<pcl::PointXYZI>());
  ...
  planner_cloud_ = std::make_unique<pointcloud_utils_ns::PCLCloud<PlannerCloudPointType>>(nh, "logging/planner_cloud",
                                                                                          world_frame_id);
  ...
  uncovered_cloud_ =
      std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "logging/uncovered_cloud", world_frame_id);
  uncovered_frontier_cloud_ = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
      nh, "logging/uncovered_frontier_cloud", world_frame_id);
  frontier_cloud_ =
...
void PlanningEnv::UpdateCollisionCloud()
{
  ...
  if (!collision_cloud_->points.empty())
  {
    collision_cloud_kdtree_->setInputCloud(collision_cloud_);
  }
}
...
void PlanningEnv::AddToCollisionCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud)
{
...
bool PlanningEnv::InCollision(double x, double y, double z) const
{
  ...
  pcl::PointXYZI check_point;
  check_point.x = x;
  check_point.y = y;
  check_point.z = z;
  std::vector<int> neighbor_indices;
  std::vector<float> neighbor_sqdist;
  collision_cloud_kdtree_->radiusSearch(check_point, parameters_.kKeyposeGraphCollisionCheckRadius, neighbor_indices,
                                        neighbor_sqdist);
