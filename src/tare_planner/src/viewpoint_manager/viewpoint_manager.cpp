...
void ViewPointManager::GetVisualizationCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr& vis_cloud)
{
  ...
  for (int i = 0; i < vp_.kViewPointNumber; i++)
  {
    if (IsViewPointCandidate(i, true))
    {
      ...
      {
        vis_point.intensity = GetViewPointCoveredPointNum(i, true) + GetViewPointCoveredFrontierPointNum(i, true);
    ...
    const pcl::PointCloud<pcl::PointXYZI>::Ptr& collision_cloud)
{
  ...
  SetViewPointCollision(robot_viewpoint_ind, false);
}
...
void ViewPointManager::UpdateViewPointVisited(const std::vector<Eigen::Vector3d>& positions, double range)
{
  if (!initialized_)
  {
    return;
  ...
  for (const auto& position : positions)
  {
    for (auto& viewpoint : viewpoints_)
    {
      ...
      double z_diff = std::abs(viewpoint_position.z - position.z());
      if (z_diff > 2)
      {
        continue;
      }
      double dist_to_position =
          (Eigen::Vector3d(viewpoint_position.x, viewpoint_position.y, viewpoint_position.z) - position).norm();
      if (dist_to_position < range)
      {
...
void ViewPointManager::UpdateViewPointVisited(std::unique_ptr<grid_world_ns::GridWorld> const& grid_world)
{
  for (int i = 0; i < viewpoints_.size(); i++)
  {
    ...
    if (grid_world->IndInBound((cell_ind)))
    {
      ...
      if (cell_status == grid_world_ns::CellStatus::COVERED_BY_OTHERS || !grid_world->IsCellTraversable(cell_ind))
      {
        SetViewPointVisited(i, true, true);
      }
      else if (cell_status == grid_world_ns::CellStatus::EXPLORING_BY_OTHERS && grid_world->IsCellTraversable(cell_ind))
      {
        SetViewPointVisited(i, false, true);
      }
    }
  }
}
...
bool ViewPointManager::AtXYBoundary(int viewpoint_ind)
{
  Eigen::Vector3i sub = grid_->Ind2Sub(viewpoint_ind);
  if (sub.x() == 0 || sub.x() == vp_.kNumber.x() - 1 || sub.y() == 0 || sub.y() == vp_.kNumber.y() - 1)
  {
    return true;
  }
  return false;
...
void ViewPointManager::SetViewPointAndNeighborsCollision(const Eigen::Vector3d& position, bool in_collision)
{
  Eigen::Vector3i viewpoint_sub = GetViewPointSub(position);
  if (grid_->InRange(viewpoint_sub))
  {
    int viewpoint_ind = grid_->Sub2Ind(viewpoint_sub);
    SetViewPointVisited(viewpoint_ind, in_collision);
    SetViewPointCollision(viewpoint_ind, in_collision);
    for (const auto& neighbor_viewpoint_ind : in_range_neighbor_indices_[viewpoint_ind])
    {
      MY_ASSERT(grid_->InRange(neighbor_viewpoint_ind));
      SetViewPointVisited(neighbor_viewpoint_ind, in_collision);
      SetViewPointCollision(neighbor_viewpoint_ind, in_collision);
    }
  }
}
...
bool ViewPointManager::IsViewPointFrontier(int viewpoint_ind)
{
  if (AtXYBoundary(viewpoint_ind))
  {
    return true;
  }
  for (int i = 0; i < connected_neighbor_indices_[viewpoint_ind].size(); i++)
  {
    int neighbor_viewpoint_ind = connected_neighbor_indices_[viewpoint_ind][i];
    if (!ViewPointInCollision(neighbor_viewpoint_ind) && !IsViewPointCandidate(neighbor_viewpoint_ind))
    {
      return true;
    }
  }
...
void ViewPointManager::UpdateCandidateViewPointCellStatus(std::unique_ptr<grid_world_ns::GridWorld> const& grid_world)
{
  for (const auto& ind : candidate_indices_)
  {
    ...
    if (grid_world->IndInBound(cell_ind))
    {
      // if (!grid_world->IsCellHasExploringPriority(cell_ind))
      // {
