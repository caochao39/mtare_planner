...
{
KeyposeNode::KeyposeNode(double x, double y, double z, int node_ind, bool is_keypose)
  : cell_ind_(0), node_ind_(node_ind), is_keypose_(is_keypose), is_connected_(true)
{
  position_.x = x;
  position_.y = y;
  position_.z = z;
...
KeyposeNode::KeyposeNode(const geometry_msgs::Point& point, int node_ind, bool is_keypose)
  : KeyposeNode(point.x, point.y, point.z, node_ind, is_keypose)
{
}

std::ostream& operator<<(std::ostream& out, const KeyposeNode& node)
{
  out << node.node_ind_ << "\t" << node.keypose_id_ << "\t" << node.cell_ind_ << "\t" << node.is_keypose_ << "\t"
      << node.is_connected_ << "\t" << node.position_.x << "\t" << node.position_.y << "\t" << node.position_.z << "\t"
      << node.offset_to_keypose_.x << "\t" << node.offset_to_keypose_.y << "\t" << node.offset_to_keypose_.z;
  return out;
}

std::istream& operator>>(std::istream& in, KeyposeNode& node)
{
  in >> node.node_ind_ >> node.keypose_id_ >> node.cell_ind_ >> node.is_keypose_ >> node.is_connected_ >>
      node.position_.x >> node.position_.y >> node.position_.z >> node.offset_to_keypose_.x >>
      node.offset_to_keypose_.y >> node.offset_to_keypose_.z;
  return in;
}
...
void KeyposeGraph::LoadFromFile(std::string nodes_filename, std::string edges_filename)
{
  misc_utils_ns::ReadVectorFromFile<KeyposeNode>(nodes_, nodes_filename);
  misc_utils_ns::Read2DVectorFromFile<int>(graph_, edges_filename);
  // Populate the distance between nodes and all the clouds
  dist_.clear();
  for (int i = 0; i < graph_.size(); i++)
  {
    ...
    {
      int to_node_index = graph_[i][j];
      double node_to_node_distance = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
          nodes_[from_node_index].position_, nodes_[to_node_index].position_);
      dist_vec.push_back(node_to_node_distance);
    }
    dist_.push_back(dist_vec);
  }

  UpdateNodes();
}

void KeyposeGraph::SetCurrentKeypose(const nav_msgs::Odometry& keypose)
{
  current_keypose_position_ = keypose.pose.pose.position;
  current_keypose_id_ = static_cast<int>(keypose.pose.covariance[0]);
}

void KeyposeGraph::AddNode(const geometry_msgs::Point& position, int node_ind, bool is_keypose)
{
  KeyposeNode new_node(position, node_ind, is_keypose);
  new_node.SetCurrentKeyposePosition(current_keypose_position_);
  new_node.SetCurrentKeyposeID(current_keypose_id_);
...
void KeyposeGraph::AddNodeAndEdge(const geometry_msgs::Point& position, int node_ind, bool is_keypose,
                                  int connected_node_ind, double connected_node_dist)
{
  AddNode(position, node_ind, is_keypose);
...
int KeyposeGraph::AddNonKeyposeNode(const geometry_msgs::Point& new_node_position)
{
  ...
  AddNode(new_node_position, new_node_index, false);
...
int KeyposeGraph::GetKeyposeNodeCount()
{
  ...
  return keypose_node_count;
}

int KeyposeGraph::GetConnectedNodeCount()
{
...
void KeyposeGraph::GetNodes(std::vector<KeyposeNode>& nodes)
{
  nodes.clear();
  nodes = nodes_;
}

void KeyposeGraph::GetEdges(std::vector<std::vector<int>>& edges)
{
  edges.clear();
  edges = graph_;
}
...
void KeyposeGraph::CheckConnectivity(const geometry_msgs::Point& robot_position)
{
  ...
  // std::cout << "connected node indices: " << std::endl;
...
int KeyposeGraph::AddKeyposeNode(const geometry_msgs::Point& keypose_position,
                                 const planning_env_ns::PlanningEnv& planning_env,
                                 const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager)
{
  int nearest_viewpoint_candidate_ind = viewpoint_manager->GetNearestCandidateViewPointInd(
      Eigen::Vector3d(keypose_position.x, keypose_position.y, keypose_position.z));
  geometry_msgs::Point nearest_viewpoint_candidate_position =
      viewpoint_manager->GetViewPointPosition(nearest_viewpoint_candidate_ind);

  int new_node_ind = GetNodeCount();
  if (new_node_ind == 0 || GetKeyposeNodeCount() == 0)
  {
    // The first node or first keypose node
    AddNode(nearest_viewpoint_candidate_position, new_node_ind, true);
    return new_node_ind;
  }
  else
  {
    ...
    for (int i = 0; i < nodes_.size(); i++)
    {
      ...
      {
        if (std::abs(nodes_[i].position_.z - nearest_viewpoint_candidate_position.z) > kAddEdgeVerticalThreshold)
        {
          continue;
        }
      }
      double dist = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
          nodes_[i].position_, nearest_viewpoint_candidate_position);
    ...
    if (min_dist_ind >= 0 && min_dist_ind < nodes_.size())
    {
      if (min_dist > kAddNodeMinDist)
      {
            ...
            last_keypose_ind < nodes_.size())
        {
          ...
          AddNodeAndEdge(nearest_viewpoint_candidate_position, new_node_ind, true, last_keypose_ind, last_keypose_dist);
        }
        else
        {
          // Add edge to the nearest node
          AddNodeAndEdge(nearest_viewpoint_candidate_position, new_node_ind, true, min_dist_ind, min_dist);
        }
        // Check other nodes
        if (!in_range_node_indices.empty())
        {
          for (int idx = 0; idx < in_range_node_indices.size(); idx++)
          {
            ...
            if (in_range_ind >= 0 && in_range_ind < nodes_.size())
            {
              ...
              double diff_x = neighbor_node.position_.x - nearest_viewpoint_candidate_position.x;
              double diff_y = neighbor_node.position_.y - nearest_viewpoint_candidate_position.y;
              double diff_z = neighbor_node.position_.z - nearest_viewpoint_candidate_position.z;
              ...
              for (int i = 0; i < check_point_num; i++)
              {
                ...
                double check_point_x = nearest_viewpoint_candidate_position.x +
                                       kAddEdgeCollisionCheckResolution * i * diff_x / neighbor_node_dist;
                double check_point_y = nearest_viewpoint_candidate_position.y +
                                       kAddEdgeCollisionCheckResolution * i * diff_y / neighbor_node_dist;
                double check_point_z = nearest_viewpoint_candidate_position.z +
...
void KeyposeGraph::UpdateNodePositionWithLoopClosure(const nav_msgs::Path& keypose_path)
{
  for (auto& node : nodes_)
  {
    int keypose_id = node.keypose_id_;
    if (keypose_id >= 0 && keypose_id < keypose_path.poses.size())
    {
      node.position_.x = keypose_path.poses[keypose_id].pose.position.x + node.offset_to_keypose_.x;
      node.position_.y = keypose_path.poses[keypose_id].pose.position.y + node.offset_to_keypose_.y;
      node.position_.z = keypose_path.poses[keypose_id].pose.position.z + node.offset_to_keypose_.z;
    }
    else
    {
      ROS_ERROR_STREAM("KeyposeGraph::UpdateNodePositionWithLoopClosure: keypose id "
                       << keypose_id << " out of bound [0, " << (keypose_path.poses.size() - 1) << "]");
    }
  }
}
