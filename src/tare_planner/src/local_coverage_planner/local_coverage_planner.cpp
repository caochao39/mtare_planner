                                                    ...
                                                    const std::unique_ptr<grid_world_ns::GridWorld>& grid_world)
{
  ...
  if (!global_path.nodes_.empty())
  {
    if (viewpoint_manager_->InLocalPlanningHorizon(global_path.nodes_.front().position_) ||
        global_path.nodes_.front().type_ == exploration_path_ns::NodeType::GLOBAL_ROADMAP ||
        global_path.nodes_.front().type_ == exploration_path_ns::NodeType::ROBOT)
    {
      for (int i = 0; i < global_path.nodes_.size(); i++)
      {
        ...
        Eigen::Vector3d node_position = global_path.nodes_[i].position_;
        ...
        {
          if (viewpoint_manager_->InLocalPlanningHorizon(node_position))
          {
            boundary_viewpoint_index = viewpoint_manager_->GetNearestCandidateViewPointInd(node_position);
          }
          else
          {
            break;
        ...
        else if (node_type == exploration_path_ns::NodeType::GLOBAL_ROADMAP)
        {
          ...
          std::vector<int> viewpoint_indices = grid_world->GetCellViewPointIndices(cell_id);
          if (!grid_world->IsNeighbor(cell_id) || viewpoint_indices.empty())
          {
            break;
          }
          else
          {
            // Get the viewpoint that is closest to the next cell and at the boundary
            if (i < global_path.nodes_.size() - 1)
            {
              int next_cell_id = global_path.nodes_[i + 1].global_subspace_index_;
              geometry_msgs::Point next_cell_center = grid_world->GetCellPosition(next_cell_id);
              boundary_viewpoint_index = viewpoint_indices.front();
              double min_dist = DBL_MAX;
              for (int j = 0; j < viewpoint_indices.size(); j++)
              {
                int viewpoint_index = viewpoint_indices[j];
                if (!viewpoint_manager_->IsViewPointFrontier(viewpoint_index))
                {
                  continue;
                }
                geometry_msgs::Point viewpoint_position = viewpoint_manager_->GetViewPointPosition(viewpoint_index);
                double dist = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
                    next_cell_center, viewpoint_position);
                if (dist < min_dist)
                {
                  min_dist = dist;
                  boundary_viewpoint_index = viewpoint_index;
                }
              }
            }
            else
...
void LocalCoveragePlanner::GetBoundaryViewpointIndices(exploration_path_ns::ExplorationPath global_path,
                                                       const std::unique_ptr<grid_world_ns::GridWorld>& grid_world)
{
  start_viewpoint_ind_ = GetBoundaryViewpointIndex(global_path, grid_world);
  global_path.Reverse();
  end_viewpoint_ind_ = GetBoundaryViewpointIndex(global_path, grid_world);
}
                                                         ...
                                                         const std::unique_ptr<grid_world_ns::GridWorld>& grid_world)
{
  ...
  GetBoundaryViewpointIndices(global_path, grid_world);
...

{
  ...
  if (has_start_end_dummy && has_robot_lookahead_dummy)
  {
    ...
    for (int i = 0; i < selected_viewpoint_indices.size(); i++)
    {
      ...
      {
        distance_matrix[i][start_end_dummy_node_ind] = misc_utils_ns::INF_DISTANCE;
        distance_matrix[start_end_dummy_node_ind][i] = misc_utils_ns::INF_DISTANCE;
      }
      ...
      {
        distance_matrix[i][robot_lookahead_dummy_node_ind] = misc_utils_ns::INF_DISTANCE;
        distance_matrix[robot_lookahead_dummy_node_ind][i] = misc_utils_ns::INF_DISTANCE;
      }
    }

    distance_matrix[start_end_dummy_node_ind][robot_lookahead_dummy_node_ind] = misc_utils_ns::INF_DISTANCE;
    distance_matrix[robot_lookahead_dummy_node_ind][start_end_dummy_node_ind] = misc_utils_ns::INF_DISTANCE;
  }
  else if (has_start_end_dummy)
  {
    ...
    for (int i = 0; i < selected_viewpoint_indices.size(); i++)
    {
      ...
      {
        distance_matrix[i][end_node_ind] = misc_utils_ns::INF_DISTANCE;
        distance_matrix[end_node_ind][i] = misc_utils_ns::INF_DISTANCE;
      }
    }
  }
  ...
  data.depots.push_back(operations_research::RoutingIndexManager::NodeIndex{ start_ind });
    ...
    const std::unique_ptr<grid_world_ns::GridWorld>& grid_world)
{
  ...
  std::vector<int> navigation_viewpoint_indices;
  GetNavigationViewPointIndices(global_path, navigation_viewpoint_indices, grid_world);
  ...
  for (auto& viewpoint_array_ind : last_selected_viewpoint_array_indices_)
  {
    if (viewpoint_manager_->ViewPointVisited(viewpoint_array_ind, true) ||
        ...
        !viewpoint_manager_->ViewPointInExploringCell(viewpoint_array_ind, true))
    {
      continue;
    }
...
LocalCoveragePlanner::FollowGlobalPath(const exploration_path_ns::ExplorationPath& global_path,
                                       const std::unique_ptr<grid_world_ns::GridWorld>& grid_world)
{
  exploration_path_ns::ExplorationPath local_path;

  std::vector<int> navigation_viewpoint_indices;  // Start, end, robot, lookahead
  GetNavigationViewPointIndices(global_path, navigation_viewpoint_indices, grid_world);

  for (int i = 0; i < navigation_viewpoint_indices.size() - 1; i++)
  {
    int cur_ind = navigation_viewpoint_indices[i];
    int next_ind = navigation_viewpoint_indices[i + 1];

    geometry_msgs::Point cur_node_position = viewpoint_manager_->GetViewPointPosition(cur_ind);
    exploration_path_ns::Node cur_node(cur_node_position, exploration_path_ns::NodeType::LOCAL_VIEWPOINT);
    cur_node.local_viewpoint_ind_ = cur_ind;
    cur_node.type_ = GetLocalPathNodeType(cur_ind, exploration_path_ns::NodeType::LOCAL_VIEWPOINT);
    local_path.Append(cur_node);
    ...
    exploration_path_ns::Node next_node(next_node_position, exploration_path_ns::NodeType::LOCAL_VIEWPOINT);
    next_node.local_viewpoint_ind_ = next_ind;
    next_node.type_ = GetLocalPathNodeType(next_ind, exploration_path_ns::NodeType::LOCAL_VIEWPOINT);
...
exploration_path_ns::NodeType
LocalCoveragePlanner::GetLocalPathNodeType(int viewpoint_index, exploration_path_ns::NodeType default_node_type)
{
  if (viewpoint_index == robot_viewpoint_ind_)
  {
    return exploration_path_ns::NodeType::ROBOT;
  }
  else if (viewpoint_index == lookahead_viewpoint_ind_)
  {
    return exploration_path_ns::NodeType::LOOKAHEAD_POINT;
  }
  else if (viewpoint_index == start_viewpoint_ind_)
  {
    return exploration_path_ns::NodeType::LOCAL_PATH_START;
  }
  else if (viewpoint_index == end_viewpoint_ind_)
  {
    return exploration_path_ns::NodeType::LOCAL_PATH_END;
  }
  else
  {
